...
auc_baseline = [0.644, 0.622, 0.650, 0.661, 0.632]  # Baseline的AUC值
auc_taβ_bphg = [0.679, 0.657, 0.661, 0.695, 0.661]  # Tαβ-BPHG的AUC值
auc_no_gnn = [0.667, 0.651, 0.646, 0.680, 0.646]  # No_GNN的AUC值
...
# 设置图表标题和轴标签
plt.title('NetTCR_strict', fontsize=12)

plt.ylabel('AUC', fontsize=12)
plt.xticks(fontsize=12, rotation=45)  # 调整X轴刻度字体大小并倾斜标签  # 调整X轴刻度字体大小
plt.yticks(fontsize=12)  # 调整Y轴刻度字体大小
# plt.text(-0.05, 1.05, 'C', fontsize=14, fontweight='bold', va='center', ha='center', transform=plt.gca().transAxes)
...
# 保存图像
save_path = os.path.join(save_dir, 'XiaorongnetStrict_auc_boxplot.png')
