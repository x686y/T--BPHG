...
auc_baseline = [0.820, 0.824, 0.812, 0.833, 0.849]  # Baseline的AUC值
auc_taβ_bphg = [0.862, 0.860, 0.860, 0.861, 0.861]  # Tαβ-BPHG的AUC值
auc_no_gnn = [0.853, 0.860, 0.861, 0.860, 0.864]  # No_GNN的AUC值
...
# 设置图表标题和轴标签
plt.title('NetTCR_full', fontsize=12)

plt.ylabel('AUC', fontsize=12)
plt.xticks(fontsize=12, rotation=45)  # 调整X轴刻度字体大小并倾斜标签

plt.yticks(fontsize=12)  # 调整Y轴刻度字体大小
# 在图的左上方添加字母"B"
# plt.text(-0.05, 1.05, 'A', fontsize=14, fontweight='bold', va='center', ha='center', transform=plt.gca().transAxes)
...
# 保存图像
save_path = os.path.join(save_dir, 'Xiaorongnetfull_auc_boxplot.png')
