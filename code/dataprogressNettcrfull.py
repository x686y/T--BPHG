...
train_delimiter = '\t'
test_delimiter = '\t'
path_pickle_train = os.path.join(root, 'GuoAndXiacaiyang_global_train_dataset_CNNfeature.pickle')
path_pickle_test = os.path.join(root, 'GuoAndXiacaiyang_global_test_dataset_CNNfeature.pickle')
...
#
# ------------------------------------------
# path_pickle_train = os.path.join(root, 'nopretrainglobal_train_dataset_CNNfeature.pickle')
# path_pickle_test = os.path.join(root, 'nopretrainglobal_test_dataset_CNNfeature.pickle')
# train_path_pickle_cdr3 = os.path.join(root, 'nopretrainCNN_feature_cdr3_train.pickle')
# train_path_pickle_peptidecdr3a=os.path.join(root, 'nopretrainCNN_feature_peptidecdr3a_train.pickle')
# train_path_pickle_peptidecdr3b=os.path.join(root, 'nopretrainCNN_feature_peptidecdr3b_train.pickle')
# test_path_pickle_cdr3 = os.path.join(root, 'nopretrainCNN_feature_cdr3_test.pickle')
# test_path_pickle_peptidecdr3a=os.path.join(root, 'nopretrainCNN_feature_peptidecdr3a_test.pickle')
# test_path_pickle_peptidecdr3b=os.path.join(root, 'nopretrainCNN_feature_peptidecdr3b_test.pickle')
def TCRDataset_global(cdr3b_map, tra_peptide_map, trb_peptide_map, cdr3b_graph, tra_peptide_graph, trb_peptide_graph,
    ...
    for cb_num in cdr3b_map.values():
        ...
        # print("cdr3b_graph keys:", cdr3b_graph.keys())  # 打印 cdr3b_graph 的键
        # print("cb_key:", cb_key)  # 打印 cb_key（来自 cdr3b_map）
...
def create_dataset_global():
    ...
    train_data['cdr3'] = train_data['tra'].astype(str) + train_data['trb'].astype(str)
    train_cdr3b = list(train_data['cdr3'])

    train_data['peptidecdr3a']=train_data['tra'].astype(str) + train_data['peptide'].astype(str)
    train_peptidecdr3a = list(train_data['peptidecdr3a'])

    train_data['peptidecdr3b'] = train_data['trb'].astype(str) + train_data['peptide'].astype(str)
    ...
    # 拼接 tra_cdr3 和 trb_cdr3 列
    test_data['cdr3'] = test_data['tra'].astype(str) + test_data['trb'].astype(str)
    test_cdr3b = list(test_data['cdr3'])

    test_data['peptidecdr3a']=test_data['tra'].astype(str) + test_data['peptide'].astype(str)
    test_peptidecdr3a = list(test_data['peptidecdr3a'])

    test_data['peptidecdr3b'] = test_data['trb'].astype(str) + test_data['peptide'].astype(str)
