...
import os, sys
from keras.layers import Softmax, Multiply, Lambda, GlobalAveragePooling1D, Dropout
...
from keras.layers import Input, Dense, Conv1D, GlobalMaxPooling1D, concatenate, Flatten, Embedding,Reshape
...
model_dir = str(args.secdir) + '_' + str(args.terdir) + '_GuoAndXiacaiyang_CNNAB'  # 根据args中的secdir和terdir参数，以及字符串'_CNN'，生成一个用于标识模型存储位置的目录名。这个目录名将用于在root_model指定的根目录下创建一个子目录来保存模型。
...
print(f"Path exists: {os.path.exists(save_model_path)}")
root_history = args.hisdir  # 从args中获取用于保存训练历史记录（如损失和准确率）的根目录路径

# 文件路径设置
train_path_pickle_cdr3 = os.path.join(root,'GuoAndXiacaiyang_CNN_feature_cdr3_train.pickle')  # 分别设置了训练数据中CDR3特征和肽段特征的 pickle 文件路径。这些文件可能包含了通过某种预处理（如使用卷积神经网络CNN提取的特征）得到的特征数据，以 pickle 格式存储。pickle 是 Python 的一种序列化对象结构的方式，可以方便地将对象保存到文件中，并在需要时重新加载。
train_path_pickle_peptidecdr3a = os.path.join(root, 'GuoAndXiacaiyang_CNN_feature_peptidecdr3a_train.pickle')
train_path_pickle_peptidecdr3b = os.path.join(root, 'GuoAndXiacaiyang_CNN_feature_peptidecdr3b_train.pickle')


test_path_pickle_cdr3 = os.path.join(root, 'GuoAndXiacaiyang_CNN_feature_cdr3_test.pickle')
test_path_pickle_peptidecdr3a = os.path.join(root, 'GuoAndXiacaiyang_CNN_feature_peptidecdr3a_test.pickle')
test_path_pickle_peptidecdr3b = os.path.join(root, 'GuoAndXiacaiyang_CNN_feature_peptidecdr3b_test.pickle')
...
# tcrB_train = enc_list_bl_max_len(train_data.trb_cdr3, blosum50_20aa, 20)
# tcra_train = enc_list_bl_max_len(train_data.tra_cdr3, blosum50_20aa, 20)  # 调用enc_list_bl_max_len函数对train_data.cdr3中的TCRB CDR3序列进行编码，假设该函数接受三个参数：序列列表、BLOSUM矩阵、最大序列长度，并返回一个编码后的数组，其中所有序列都被填充或截断到最大长度，得到训练集的TCRB CDR3编码（tcrb_train）print(type(tcrb_train))  # 输出类型信息

tcrB_train = enc_list_bl_max_len(train_data.trb, blosum62_20aa, 40)
tcra_train = enc_list_bl_max_len(train_data.tra, blosum62_20aa, 40)
...
# print(f"tcrb_test shape: {tcrbb_test.shape}")
...
def CNN_extra():
    ...
    #
    # # 定义卷积层并显式命名
    # cdr_conv1 = Conv1D(16, 1, padding='same', activation='sigmoid', kernel_initializer='glorot_normal',
    #                    name='cdr_conv1')(cdra_in)
    # cdr_conv3 = Conv1D(16, 3, padding='same', activation='sigmoid', kernel_initializer='glorot_normal',
    #                    name='cdr_conv3')(cdra_in)
    # cdr_conv5 = Conv1D(16, 5, padding='same', activation='sigmoid', kernel_initializer='glorot_normal',
    #                    name='cdr_conv5')(cdra_in)
    # cdr_conv7 = Conv1D(16, 7, padding='same', activation='sigmoid', kernel_initializer='glorot_normal',
    #                    name='cdr_conv7')(cdra_in)
    # cdr_conv9 = Conv1D(16, 9, padding='same', activation='sigmoid', kernel_initializer='glorot_normal',
    #                    name='cdr_conv9')(cdra_in)
    #
    # cdrb_conv1 = Conv1D(16, 1, padding='same', activation='sigmoid', kernel_initializer='glorot_normal',
    #                     name='cdrb_conv1')(cdrb_in)
    #
    # cdrb_conv3 = Conv1D(16, 3, padding='same', activation='sigmoid', kernel_initializer='glorot_normal',
    #                     name='cdrb_conv3')(cdrb_in)
    # cdrb_conv5 = Conv1D(16, 5, padding='same', activation='sigmoid', kernel_initializer='glorot_normal',
    #                     name='cdrb_conv5')(cdrb_in)
    # cdrb_conv7 = Conv1D(16, 7, padding='same', activation='sigmoid', kernel_initializer='glorot_normal',
    #                     name='cdrb_conv7')(cdrb_in)
    # cdrb_conv9 = Conv1D(16, 9, padding='same', activation='sigmoid', kernel_initializer='glorot_normal',
    #                     name='cdrb_conv9')(cdrb_in)
    #
    # # pep 网络部分
    ...
    #
    # # 拼接层，现在包括基因信息
    # cdr3a_cat = concatenate([cdr_conv1,cdr_conv3, cdr_conv5, cdr_conv7, cdr_conv9],
    #                         name='cdr3a_cat')
    # print(" cdr3a_cat shape:",  cdr3a_cat.shape)
    # cdr3b_cat = concatenate([cdrb_conv1, cdrb_conv3, cdrb_conv5, cdrb_conv7, cdrb_conv9],
    #                         name='cdr3b_cat')
    # print(" cdr3b_cat shape:", cdr3b_cat.shape)
    # pep_cat = concatenate([pep_conv1, pep_conv3, pep_conv5, pep_conv7, pep_conv9],
    #                       name='pep_cat')#新加
    # print(" pep_cat shape:", pep_cat.shape)
    ...
    # 密集层
    # dense1= Dense(256, activation='relu')(final_features)
    # dense2= Dense(128, activation='relu')(dense1)
    #
    # dense1= Dense(128, activation='relu')(final_features)
    # dense2= Dense(64, activation='relu')(dense1)
    #
    # dropout = Dropout(0.5)(dense2)#子注意力和交叉注意串行时候加入，不然要训练1000次
    #
    # # 输出层
...
# Define specificity as a custom metric
def specificity(y_true, y_pred, threshold=0.5):
    y_pred_class = K.cast(K.greater_equal(y_pred, threshold), K.floatx())
    true_negatives = K.sum(K.cast(K.equal(y_true, 0) & K.equal(y_pred_class, 0), K.floatx()))
    false_positives = K.sum(K.cast(K.equal(y_true, 0) & K.equal(y_pred_class, 1), K.floatx()))
...
#     classes=np.unique(y_train),  # 类别的唯一值
#     y=y_train  # 标签数据
# )
#
# # 将类别权重转换为字典格式
...
#
#     # 应用类别权重（使用tf.gather，根据y_true来获取对应的类别权重）
#     weights = tf.gather(class_weights_tensor, K.cast(y_true, 'int32'))
...
#
# # 使用加权的二元交叉熵损失函数编译模型
# model = CNN_extra()  # 假设这是你的模型定义
# model.compile(
#     loss=weighted_binary_crossentropy,
#     optimizer=tf.keras.optimizers.Adam(learning_rate=lr),  # 假设lr已定义
#     metrics=[tf.keras.metrics.BinaryAccuracy(),
#              tf.keras.metrics.AUC(curve="ROC", name="ROCAUC"),
#              specificity,
#              tf.keras.metrics.AUC(curve="PR", name="F1"),
...
model.compile(loss="binary_crossentropy",
              ...
              metrics=[tf.keras.metrics.BinaryAccuracy(), tf.keras.metrics.AUC(curve="ROC", name="ROCAUC"),  specificity,tf.keras.metrics.AUC(curve="PR", name="F1"),tf.keras.metrics.Precision(), tf.keras.metrics.Recall()])
...
# You can extend the comparison as needed for further analysis
...
specificity = history_dict['specificity']
f1_score = history_dict['F1']  # Added f1_score for validation set
precision = history_dict['precision']  # Precision for validation
recall = history_dict['recall']  # Recall for validation
val_loss = history_dict['val_loss']
val_acc = history_dict['val_binary_accuracy']
val_auc_roc = history_dict['val_ROCAUC']
val_specificity = history_dict['val_specificity']
val_f1_score = history_dict['val_F1']  # Added f1_score for validation set
val_precision = history_dict['val_precision']  # Precision for validation
val_recall = history_dict['val_recall']  # Recall for validation
...
dfhistory = {'epoch': epoch,
             'loss': loss, 'acc': acc, 'auc_roc': auc_roc, 'specificity': specificity,'F1':f1_score,'precision':precision,'recall':recall,
             'val_loss': val_loss, 'val_acc': val_acc,
             'val_auc_roc': val_auc_roc, 'val_specificity':val_specificity,'val_F1':val_f1_score,'val_precision':val_precision,'val_recall':val_recall}  # 创建了一个字典dfhistory，它包含了所有要保存到DataFrame中的列名和对应的数据。列名包括'epoch'、'loss'、'acc'、'auc_roc'、'val_loss'、'val_acc'和'val_auc_roc'
df = pd.DataFrame(
...
#             # new_file_name = f"minloss_AUC_epoch{loss_num_epoch:03d}.hdf5"
...
#             # Define the new file name
#             # new_file_name = f"max_AUC_epoch{num_epoch:03d}.hdf5"
...
# 更新字典保存注意力机制后的特征
cdr3b_dict_train = {}
for i in range(len(train_data.tra)):
    if train_data.tra[i] + train_data.trb[i] not in cdr3b_dict_train.keys():
        cdr3b_dict_train[train_data.tra[i] + train_data.trb[i]] =cat_cdr3_attention_output_train[i]

peptide_cdr3a_dict_train = {}
for i in range(len(train_data.peptide)):
    if train_data.tra[i] + train_data.peptide[i] not in peptide_cdr3a_dict_train.keys():
        peptide_cdr3a_dict_train[train_data.tra[i] + train_data.peptide[i]] =  cat_peptidecdr3a_attention_output_train[i]

peptide_cdr3b_dict_train = {}
for i in range(len(train_data.peptide)):
    if train_data.trb[i]+train_data.peptide[i] not in peptide_cdr3b_dict_train.keys():
        peptide_cdr3b_dict_train[train_data.trb[i]+train_data.peptide[i]] =  cat_peptidecdr3b_attention_output_train[i]

cdr3b_dict_test = {}
for i in range(len(test_data.tra)):
    if test_data.tra[i] + test_data.trb[i] not in cdr3b_dict_test.keys():
        cdr3b_dict_test[test_data.tra[i] + test_data.trb[i]] = cat_cdr3_attention_output_test[i]

peptide_cdr3a_dict_test = {}
for i in range(len(test_data.peptide)):
    if test_data.tra[i] + test_data.peptide[i] not in peptide_cdr3a_dict_test.keys():
        peptide_cdr3a_dict_test[test_data.tra[i] + test_data.peptide[i]] = cat_peptidecdr3a_attention_output_test[i]

peptide_cdr3b_dict_test = {}
for i in range(len(test_data.peptide)):
    if test_data.trb[i]+test_data.peptide[i] not in peptide_cdr3b_dict_test.keys():
        peptide_cdr3b_dict_test[test_data.trb[i]+test_data.peptide[i]] = cat_peptidecdr3b_attention_output_test[i]
...
# 查找以 `max_AUC_` 开头的模型文件
def find_max_auc_model(save_model_path):
    for root, dirs, files in os.walk(save_model_path):
        for file in files:
            if file.startswith("max_AUC_"):
                return os.path.join(root, file)  # 返回完整的文件路径
    return None  # 如果找不到，返回 None

# 调用函数查找 `max_AUC_` 开头的文件
max_auc_model_path = find_max_auc_model(save_model_path)

# 重新构建模型架构
best_auc_model = CNN_extra()  # 确保 CNN_extra() 函数与原始模型结构一致
best_auc_model.compile(
...
# 将权重加载到模型中
print(f"Loading weights from {max_auc_model_path}...")
best_auc_model.load_weights(max_auc_model_path)

# 使用测试集进行预测
print("Predicting on test data...")


predictions = best_auc_model.predict(validation_inputs)  # 使用 validation_inputs 即测试集输入
...
results_df = pd.DataFrame({
    ...
    'true_label': test_data['Binding'],  # 真实标签
    'probability': predictions.flatten()  # 模型预测概率
})

# 保存预测结果为TSV文件
output_file = os.path.join(root2, "IMMREPNoGNN_pred_fold0.tsv")  # 结果文件路径
# output_file = os.path.join(root2, "nettcr_balNoGNN_pred_fold4.tsv")  # 结果文件路径
# output_file = os.path.join(root2, "nettcr_strict4_5foldsNoGNN_pred_fold4.tsv")  # 结果文件路径

# output_file = os.path.join(root2, "nettcrfullNoGNN_pred_fold3.tsv")  # 结果文件路径
# output_file = os.path.join(root2, "Public2_GuoAndXiacaiyangNoGNN_pred_fold0.tsv")  # 结果文件路径
# 确保输出目录存在
output_dir = os.path.dirname(output_file)  # 获取输出文件的目录
if not os.path.exists(output_dir):  # 检查目录是否存在
    os.makedirs(output_dir)  # 如果不存在，则创建目录
print(f"Saving predictions to {output_file}...")
results_df.to_csv(output_file, header=True, sep='\t', index=False)
